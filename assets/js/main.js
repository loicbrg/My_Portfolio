...
// Variable pour stocker l'état du mode sombre
var darkModeEnabled;

// Fonction appelée lorsque l'élément avec l'ID "darkTrigger" est cliqué
$("#darkTrigger").click(function(){
	// Vérifie si le corps (body) a déjà la classe "dark"
  if ($("body").hasClass("dark")){
	...
	// Si le corps n'a pas la classe "dark", la classe "dark" est ajoutée au corps, activant le mode sombre
    $("body").addClass("dark");
...
// Fonction exécutée lorsque le document est prêt (chargement initial de la page)
$(document).ready(function () {
  // Récupère l'état du mode sombre depuis le stockage local
  darkModeEnabled = getDarkModeState();

  // Vérifie si le dark mode n'est pas set
  if (darkModeEnabled === null) {
    var d = new Date();
    var n = d.getHours();
    darkModeEnabled = (n > 17 || n < 8);
    saveDarkModeState();
  }
   // Si dark mode set, on ajoute la classe dark au body
  if (darkModeEnabled) {
    $("body").addClass("dark");
  }
});
